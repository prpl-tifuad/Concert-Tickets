<?php
  ...
  if(isset($_GET['cari'])){
    $cari = $_GET['cari'];
    $QueryString = "SELECT * FROM konser WHERE namakonser LIKE '%$cari%' OR tanggal LIKE '%$cari%' OR namatempat LIKE '%$cari%' ";
    $SQL = mysqli_query($connect,$QueryString);     
  }else{
     $QueryString = "SELECT * FROM konser";
  $SQL = mysqli_query($connect,$QueryString);  
  }
...
<div>
  <br>
    ...
    <div class="row">
      ...
      <div class="col-md-3">
        <div class="thumbnail">
              <img src="<?= $data['poster']; ?>" alt="Lights" style="width:100%">
            </div>
            <div class="caption">
              <a href="pembelian.php?kodekonser=<?= $data['kodekonser']; ?>"><p><?= $data['namakonser'] ?></p></a>
          </div>       
      </div>
